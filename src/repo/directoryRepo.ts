...
export async function listClinicsRemote(): Promise<ClinicOption[]> {
...
export async function listDentistsRemote(options?: { clinicId?: string }): Promise<DentistOption[]> {
  const dentists = await listDentistsFirebase({
    includeDeleted: false,
    includeInactive: false,
  })
  return dentists.filter((dentist) => !options?.clinicId || dentist.clinicId === options.clinicId).map((dentist) => ({
    id: dentist.id,
    name: dentist.name,
